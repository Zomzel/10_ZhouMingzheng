...
from aemodel_adone import AutoEncoder

def parse_args():
    parser = argparse.ArgumentParser(description="DONE embeddings")
    parser.add_argument('--config', nargs='?', default='config_adone', help='Input config path')
    return parser.parse_args()

def load_config(fname='./config_adone'):
...
def trainer(sess, model, x_train, y_train, num_epochs):
    with sess.as_default():
        ...
        for feed_dict, order, epoch_end in batcher:
            ...
            model.train_step(sess, feed_dict, start_align = (epoch > config['pretrain_threshold']),
                             train_disc = (epoch % config["train_disc_every"] == 0 and epoch < config["disc_stop"]), print_this = False)
            if epoch_end:
        ...
        finalcsv = pd.DataFrame(final)
        finalcsv.to_csv("adone_emb.csv")
